...
function fetchAnimals(){
    ...
    .then(function(data) {
        ...
        var divID = 100+i;
        var address = data.animals[i].contact.address.city +", "+ data.animals[i].contact.address.state
        var createDiv = `<div class="row" id=${divID}>
        <div>
          <a><img src="${img}"  class= "dogImage" alt="image for dog #1" alt="article preview image"></a>
        </div>
        <div>
          <h5>${name}</h5>
          <p>${description}</p>
          <button class= "dogButton" data-name= "${address}"> Find Me </button>
          <hr>
            
        </div>
        </div>`
        document.getElementById("dogCard").innerHTML += createDiv;
        }
    })
    .catch(function(err) {
...
$(document).on("click", ".dogButton", addressFunction)
function addressFunction(){
    let dogAddress = $(".dogButton").data("name")
    console.log(dogAddress)
}
//fetchAnimals();
